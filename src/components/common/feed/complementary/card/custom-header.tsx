      ...
      <div className="flex justify-between items-start w-full select-none">
        <div className="flex gap-4">
          ...
          <div className="flex flex-col">
            ...
            <span className="text-sm text-violet-400">{name ?? "--"}</span>
          </div>
        </div>

        <div className="flex gap-2 items-center">
          {opinion.isEdited ? (
            <span className="text-sm text-orange-400 uppercase">Edited</span>
          ) : (
            <></>
          )}
          {isMyOpinion ? (
            <div className="hover:bg-gray-300/10 rounded-full">
              <DropdownMenu modal={false}>
                <DropdownMenuTrigger className="p-1" aria-label="config-button">
                  <Settings />
                </DropdownMenuTrigger>
